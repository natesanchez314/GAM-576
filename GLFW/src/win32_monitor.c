    ...
    for (adapterIndex = 0;  ;  adapterIndex++)
    {
        ...
        if (hasDisplays)
        {
            for (displayIndex = 0;  ;  displayIndex++)
            {
                ...
                tmpMonitors = monitors;
                monitors = realloc(tmpMonitors, sizeof(_GLFWmonitor*) * found);
                if (monitors == 0)
                {
                    assert(monitors != 0);
                    free(tmpMonitors);
    ...
    *count = 0;
    ...
    for (;;)
    {
        ...
        if (*count == size)
        {
            ...
            tmpResult = result;
            result = (GLFWvidmode*) realloc(tmpResult, size * sizeof(GLFWvidmode));
            if (result == 0)
            {
                assert(result != 0);
                free(tmpResult);
